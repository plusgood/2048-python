...
def game_state(mat):
    """for i in xrange(len(mat)):
        for j in xrange(len(mat[0])):
            if mat[i][j]==2048:
                return 'win'"""
    for i in xrange(len(mat)-1): #intentionally reduced to check the row on the right and below
        for j in xrange(len(mat[0])-1): #more elegant to use exceptions but most likely this will be their solution
            if mat[i][j]==mat[i+1][j] or mat[i][j+1]==mat[i][j]:
                return 'not over'
    for i in xrange(len(mat)): #check for any zero entries
        for j in xrange(len(mat[0])):
            if mat[i][j]==0:
                return 'not over'
    for k in xrange(len(mat)-1): #to check the left/right entries on the last row
        if mat[len(mat)-1][k]==mat[len(mat)-1][k+1]:
            return 'not over'
    for j in xrange(len(mat)-1): #check up/down entries on last column
